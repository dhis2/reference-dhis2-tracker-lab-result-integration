...
test("should successfully import the lab report into DHIS2", async function({
                                                                                request
                                                                            }) {
    const specimenId = Math.floor(Math.random() * 10000).toString();
    const newEnrollment = await request.post("/api/tracker", {
        params: {
            "async": false
        },
        data: {
    ...
    await expect.poll(async function() {
        return (await (await request.get("/api/tracker/enrollments/" + enrollmentId, {
            params: {
                "fields": "*"
            }
        })).json()).events.filter(
            (e) => e.programStage === "mDvE6kdNpty" && e.status === "COMPLETED", ).flatMap((e) => e.dataValues).filter(
            (dv) => dv.dataElement === "TS6Yt0weEhi" && dv.value === specimenId).length
    }, {
        timeout: 40000
    }).toBe(1);
});